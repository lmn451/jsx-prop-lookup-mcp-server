...
describe('JSXPropAnalyzer', () => {
  ...
  describe('analyzeProps', () => {
    test('should analyze all files in examples directory', async () => {
      ...
      assert.ok(result.summary.totalFiles >= 4, `Should find at least 4 files, found ${result.summary.totalFiles}`);
      assert.ok(result.summary.totalComponents >= 5, `Should find at least 5 components, found ${result.summary.totalComponents}`);
      assert.ok(result.summary.totalProps >= 10, `Should find at least 10 props, found ${result.summary.totalProps}`);
    });
    ...
    test('should find Button component with expected props', async () => {
      ...
      assert.ok(buttonComponent.propsInterface === 'ButtonProps', 'Should have ButtonProps interface');
  ...
  describe('findPropUsage', () => {
    test('should find onClick prop usage', async () => {
      ...
      assert.ok(usages.length >= 4, `Should find at least 4 onClick usages, found ${usages.length}`);
      
      // Should find onClick in Button component definition
      const buttonDefUsage = usages.find(u => u.componentName === 'Button' && u.file.includes('Button.tsx'));
      assert.ok(buttonDefUsage, 'Should find onClick in Button component definition');
      
      // Should find onClick in JSX usage
      const jsxUsage = usages.find(u => u.value && (u.value.includes('handleIncrement') || u.value.includes('handleReset')));
    ...
    test('should find className prop usage', async () => {
      ...
      assert.ok(usages.length >= 3, `Should find at least 3 className usages, found ${usages.length}`);
    ...
    test('should find prop usage limited to specific component', async () => {
      ...
      const allButtonRelated = usages.every(u => 
  ...
  describe('findComponentsWithoutProp', () => {
    test('should find Select components without width prop', async () => {
      ...
      assert.ok(result.missingPropUsages.length >= 2, `Should find at least 2 Select without width, found ${result.missingPropUsages.length}`);
      
      // Summary should make sense
      assert.ok(result.summary.totalInstances >= result.summary.missingPropCount, 'Total instances should be >= missing count');
      assert.ok(result.summary.missingPropPercentage >= 0 && result.summary.missingPropPercentage <= 100, 'Percentage should be 0-100');
      ...
      result.missingPropUsages.forEach(usage => {
        ...
        assert.ok(!usage.existingProps.includes('width'), 'Should not include width in existing props');
      });
    });
    ...
    test('should handle components with spread props correctly', async () => {
      ...
      const withSpread = result.missingPropUsages.filter(u => u.existingProps.includes('...spread'));
      assert.strictEqual(withSpread.length, 0, 'Should not flag components with spread as missing width');
    });

    test('should NOT find missing props for non-existent component', async () => {
      const result = await analyzer.findComponentsWithoutProp('NonExistentComponent', 'someProp', examplesDir);
      
      assert.strictEqual(result.missingPropUsages.length, 0, 'Should not find missing props for non-existent component');
    ...
    test('should NOT find missing props when all components have the required prop', async () => {
      ...
      assert.ok(result.summary.totalInstances >= result.summary.missingPropCount, 'Summary should be consistent');
    });
  });
  ...
  describe('TypeScript interface detection', () => {
    test('should detect ButtonProps interface', async () => {
      ...
      assert.strictEqual(buttonComponent.propsInterface, 'ButtonProps', 'Should detect ButtonProps interface');
    });
    ...
    test('should detect CardProps interface', async () => {
      ...
      assert.strictEqual(cardComponent.propsInterface, 'CardProps', 'Should detect CardProps interface');
    });
  });
